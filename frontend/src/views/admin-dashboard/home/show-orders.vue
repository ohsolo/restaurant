<template>
    ...
    <div class="row mt-2">
      <div class="col-12">
        <div class="d-flex flex-wrap">
          ...
          >
            <label for="" class="mb-2">{{ $t("adminHome.Deliverer") }}</label>
            <SearchContent
          ...
          <div
            ...
            style="min-width: 150px; width: 100%; max-width: 200px"
          >
            <label for="" class="mb-2">{{ $t("adminHome.Store") }}</label>
            <SearchContent
              url="admin/get-restaurants-by-name?search="
              objKey="restaurants"
              myKey="title"
              @my-call-back="storeCallBack"
            />
          </div>
          <div
            class="input_wrapper mx-2 mt-3"
            style="min-width: 150px; width: 100%; max-width: 200px"
          >
            <label for="" class="mb-2">{{ $t("adminHome.Region") }}</label>
            <SearchContent
          ...
          <div
            ...
            style="min-width: 150px; width: 100%; max-width: 200px"
          >
            <label for="" class="mb-2">{{ $t("adminHome.Status") }}</label>
      ...
      <div class="col-lg-8 col-md-6 col-sm-12 mt-3">
        ...
        >
          <label for="" class="me-3 w-50 d-flex align-items-center">
            <span class="me-2">{{ $t("adminHome.From") }}</span>
            <input
              type="date"
              required
              class="form-control"
          ...
          <label for="" class="d-flex w-50 align-items-center">
            <span class="me-2">{{ $t("adminHome.To") }}</span>
            <input
              type="date"
              required
  ...
  <div class="row mt-4">
    <div class="col-12">
      ...
      >
        <CustomCard :num="cardsData?.card1 ?? '0'" title="adminHome.cardSm1" />
        <CustomCard :num="cardsData?.card2 ?? '0'" title="adminHome.cardSm2" />
        <CustomCard :num="cardsData?.card3 ?? '0'" title="adminHome.cardSm3" />
        <CustomCard :num="cardsData?.card4 ?? '0'" title="adminHome.cardSm4" />
      </div>
    </div>
  </div>
  <!-- Data Table -->
        ...
        <table class="table table-hover" style="min-width: 1400px">
          ...
          <tbody>
            ...
            <tr v-for="data in records?.data" :key="data?.order_id">
              ...
              <td align="center" valign="middle">
                <p class="m_0">{{ data?.restaurant?.title ?? "-" }}</p>
              </td>
              <td align="center" valign="middle">
                <small>{{ data?.address_detail?.location ?? "-" }}</small>
              </td>
              <td align="center" valign="middle">
                <p class="m_0">
              ...
              <td align="center" valign="middle">
                <router-link
                  :to="{ name: 'OrderDetails', params: { id: data.order_id } }"
...
export default {
  ...
  methods: {
    ...
    getRecords() {
      ...
      let fd = this.fd();
      API(this, "admin/get-filter-orders", "post", fd)
        .then((res) => {
    ...
    storeCallBack(item) {
      console.log(
        "🚀 ~ file: show-orders.vue ~ line 702 ~ storeCallBack ~ item",
        item
      );
      if (item == "") {
        this.filter.rest_id = "";
      } else {
        this.filter.rest_id = item.rest_id;
      }
    },
    applyFilters() {
