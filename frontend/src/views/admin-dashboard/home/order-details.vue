<template>
  <div class="container-fluid pb-5">
    <div class="row">
      <div class="col-12">
        <router-link class="border-0 rounded px-4 py-2 text-light bg_primary focus_primary" to="/">
          <fa icon="arrow-left" />
        </router-link>
        <button class="
            border-0
      ...
      <div class="col-12">
        <div class="d-flex align-items-center justify-content-end">
          <button @click="initiateChat" class="
              ...
              me-3
            " v-if="
              record?.chat_rider_started_from_admin &&
      ...
      <div class="col-md-6 col-sm-12 mt-4">
        <div class="card">
          ...
          <div class="card-body">
            ...
            <p><b>{{$t('singleOrder.Phone')}} :</b> 
              &nbsp;
            <span v-if="record?.restaurant?.phones.length">
              <template v-for="x in record?.restaurant?.phones">{{ x }},&nbsp;</template>
            </span>
            <span v-else>
              -
            </span>
    ...
    <div class="row" v-if="record?.status == 'pending' && record?.chat_rider_started_from_admin">
      <div class="col-12" id="ChatBox">
        <ChatBox sendMessageAPI="admin/add-chat-message/" getMessageAPI="admin/get-all-messages/" :ID="singleOrderID"
          :matchID="this.$store.state.admin.admin_id" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    initiateChat() {
      console.log(this.record.chat_rider_started_from_admin)
      this.$store.commit("loaderShow");
      API(this, `admin/start-chat/${this.singleOrderID}`, "post", null)
        .then((res) => {
          this.$store.commit("loaderHide");
          this.record.chat_rider_started_from_admin = true;
          setTimeout(() => {
