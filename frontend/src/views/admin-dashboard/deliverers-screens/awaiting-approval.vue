            ...
            <thead>
              <tr>
                <th class="bg-dark text-light">
                  <h5 class="m_0 text-center">{{ $t("await_App.th1") }}</h5>
                </th>
                <th class="bg-dark text-light">
                  <h5 class="m_0 text-center">{{ $t("await_App.th2") }}</h5>
                </th>
                <th class="bg-dark text-light">
                  <h5 class="m_0 text-center">{{ $t("await_App.th3") }}</h5>
                </th>
                <th class="bg-dark text-light">
                  <h5 class="m_0 text-center">{{ $t("await_App.th4") }}</h5>
                </th>
                <th class="bg-dark text-light">
                  <h5 class="m_0 text-center">{{ $t("await_App.th5") }}</h5>
                </th>
                <th class="bg-dark text-light">
                  <h5 class="m_0 text-center">{{ $t("await_App.th6") }}</h5>
                </th>
                <th class="bg-dark text-light">
                  <h5 class="m_0 text-center">{{ $t("await_App.th7") }}</h5>
                </th>
                <th class="bg-dark text-light">
                  <h5 class="m_0 text-center">{{ $t("await_App.th8") }}</h5>
                </th>
                <th class="bg-dark text-light">
                  <h5 class="m_0 text-center">{{ $t("await_App.th9") }}</h5>
                </th>
                <th class="bg-dark text-light">
                  <h5 class="m_0 text-center">{{ $t("await_App.th10") }}</h5>
                </th>
                <th class="bg-dark text-light">
                  <h5 class="m_0 text-center">{{ $t("await_App.th11") }}</h5>
                </th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td v-if="records?.data == ''" colspan="11" class="text-center">
                  <span>{{ $t("await_App.notFound") }}</span>
                </td>
              </tr>
              <tr v-for="data in records?.data" :key="data">
                ...
                <td class="text-center">
                  <button
                    ...
                    "
                    type="button"
                    @click="statusUpdate(data.rider_id)"
                    title="Update status"
                  >
                    <fa icon="check" />
                  </button>
                  ...
                  <button
                    type="button"
                    class="border-0 bg-danger text-light rounded py-1 px-2"
                    data-bs-toggle="modal"
...
export default {
  ...
  title: "Riders Awaiting Approval | Dashbaord",
  ...
  methods: {
    getRecords() {
      ...
      API(this, "admin/get-riders", "post", fd)
        ...
        .catch((error) => {
          console.error(
            "🚀 ~ file: awaiting-approval.vue ~ getRecords ~ error",
            error
          );
          this.$store.commit("loaderHide");
        });
    ...
    statusUpdate(id) {
      ...
      API(this, "admin/update-rider/" + id, "post", fd)
        .then((response) => {
        ...
        .catch((error) => {
          console.error(
            "🚀 ~ file: awaiting-approval.vue ~ statusUpdate ~ error",
            error
          );
          this.$store.commit("loaderHide");
        });
    ...
    handleTextarea(event) {
      this.blockDriver.msg = event.target.value;
    },
    blockRequest() {
      this.$store.commit("loaderShow");
      const fd = new FormData();
      ...
      API(this, "admin/block-rider", "post", fd)
        ...
        .catch((error) => {
          console.error(
            "🚀 ~ file: awaiting-approval.vue ~ blockRequest ~ error",
            error
          );
          this.$store.commit("loaderHide");
        });
